...
int main() {
  ...
  // create a chunk
  const GLuint x = 16, y = 16, z = 256;
  const GLuint size = x * y * z;
  Chunk<VertexFormat::Color>::setSize({x, y, z});
  Chunk<VertexFormat::Color>::enableOcclusionCulling(true);
  std::vector<GLuint> blockMap(size, static_cast<GLuint>(BlockType::Grass));
  Chunk<VertexFormat::Color> chunk(blockMap);
  chunk.setCoor(0, 0);

  // Render
  Render<VertexFormat::Color> render(chunk);
  render.bind();
  ...
  while (window.isAlive()) {
    ...
    // glDrawElements(GL_TRIANGLES, 1722, GL_UNSIGNED_INT, 0);
    // glDrawArrays(GL_TRIANGLES, 0, 246);
    render.draw();
